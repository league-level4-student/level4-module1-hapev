...
public class _00_SnakeGame implements ActionListener, KeyListener {
	...
	private void setFoodLocation() {
		...
		Location l = new Location((int)Math.floor(Math.random()*15), (int)Math.floor(Math.random()*12));
...
//		}
	...
	private void gameOver() {
		...
		int input = JOptionPane.showOptionDialog(null, "You died. Your score was " + snake.snake.size() +  ". Play again?", null, 0, -1, null, options, 0);
		...
		//   else, exit the game
		if (input == 0) {
			snake.reset(new Location((int)Math.floor(Math.random()*15), (int)Math.floor(Math.random()*12)));
			setFoodLocation();
			timer.start();
		}
